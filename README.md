...
<p align="center">
  ...
  <a href="#why-tnyma-ai">Why TnymaAI</a> •
...
Building AI agents shouldn't require mastering the command line. TnymaAI was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**

| Challenge | TnymaAI Solution |
|-----------|----------------|
...
Download the latest release for your platform from the [Releases](https://github.com/ValueCell-ai/TnymaAI/releases) page.
...
TnymaAI employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:
...
- Clicking the window close button (`X`) hides TnymaAI to tray; it does **not** fully quit the app. Use tray menu **Quit TnymaAI** for complete shutdown.
...
Tag pipelines now also enforce immutable `TNYMA_AI_REF` inputs, validate macOS notarization, and can optionally notify Feishu bots immediately when verify / bundle / package jobs fail and when the release job succeeds or fails. On the macOS runner, notarization can use a saved `notarytool` keychain profile through `package-mac.sh`.
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make TnymaAI better.
...
### TnymaAI Partner Program 🚀

We're launching the TnymaAI Partner Program and looking for partners who can help introduce TnymaAI to more clients, especially those with custom AI agent or automation needs.

Partners help connect us with potential users and projects, while the TnymaAI team provides full technical support, customization, and integration.
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/TnymaAI&type=Date" alt="Star History Chart" />
</p>
...
TnymaAI is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.
