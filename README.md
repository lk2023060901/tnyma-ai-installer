...
Tag pipelines now also enforce immutable `TNYMA_AI_REF` inputs, validate macOS notarization, and can optionally notify Feishu bots when builds fail and when releases succeed or fail. On the macOS runner, notarization can use a saved `notarytool` keychain profile through `package-mac.sh`.
