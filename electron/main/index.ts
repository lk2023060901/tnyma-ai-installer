...
import { ensureTnymaAIContext, repairTnymaAIOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
// On Wayland this maps the running window to tnyma-ai.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('tnyma-ai.desktop');
}
...
if (!gotElectronLock) {
  console.info('[TnymaAI] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[TnymaAI] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[TnymaAI] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
async function initialize(): Promise<void> {
  ...
  // Repair any bootstrap files that only contain TnymaAI markers (no OpenClaw
  // template content). This fixes a race condition where ensureTnymaAIContext()
  // previously created the file before the gateway could seed the full template.
  void repairTnymaAIOnlyBootstrapFiles().catch((error) => {
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second TnymaAI instance detected; redirecting to the existing window');
