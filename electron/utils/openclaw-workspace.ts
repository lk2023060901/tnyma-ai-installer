...
const TNYMA_AI_BEGIN = '<!-- tnyma-ai:begin -->';
const TNYMA_AI_END = '<!-- tnyma-ai:end -->';
...
/**
 * Merge a TnymaAI context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeTnymaAISection(existing: string, section: string): string {
  const wrapped = `${TNYMA_AI_BEGIN}\n${section.trim()}\n${TNYMA_AI_END}`;
  const beginIdx = existing.indexOf(TNYMA_AI_BEGIN);
  const endIdx = existing.indexOf(TNYMA_AI_END);
  if (beginIdx !== -1 && endIdx !== -1) {
    return existing.slice(0, beginIdx) + wrapped + existing.slice(endIdx + TNYMA_AI_END.length);
  }
...
/**
 * Detect and remove bootstrap .md files that contain only TnymaAI markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairTnymaAIOnlyBootstrapFiles(): Promise<void> {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(TNYMA_AI_BEGIN);
      const endIdx = content.indexOf(TNYMA_AI_END);
      if (beginIdx === -1 || endIdx === -1) continue;

      const before = content.slice(0, beginIdx).trim();
      const after = content.slice(endIdx + TNYMA_AI_END.length).trim();
      if (before === '' && after === '') {
        try {
          await unlink(filePath);
          logger.info(`Removed TnymaAI-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove TnymaAI-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge TnymaAI context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeTnymaAIContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('TnymaAI context directory not found, skipping context merge');
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      ...
      const merged = mergeTnymaAISection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged TnymaAI context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure TnymaAI context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureTnymaAIContext(): Promise<void> {
  let skipped = await mergeTnymaAIContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeTnymaAIContextOnce();
    if (skipped === 0) {
      logger.info(`TnymaAI context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`TnymaAI context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`TnymaAI context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
