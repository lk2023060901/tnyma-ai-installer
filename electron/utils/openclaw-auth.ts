 ...
 * Used by TnymaAI to seed the provider store when it's empty but providers are
...
/**
 * Write the TnymaAI gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged TnymaAI loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  TnymaAI sets
 ...
 * OpenClaw/TnymaAI versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When TnymaAI-managed moonshot provider exists, prefer centralized key
    ...
    // for TnymaAI to properly integrate with its updated tool system.
    ...
    // but TnymaAI historically stored them only under `channels.<type>.accounts.default`.
