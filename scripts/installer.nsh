...
!macro customUnInstall
  ...
  _cu_removeData:
    ; Kill any lingering TnymaAI processes to release file locks on electron-store
    ; JSON files (settings.json, tnyma-ai-providers.json, window-state.json, etc.)
    ${nsProcess::FindProcess} "${APP_EXECUTABLE_FILENAME}" $R0
    ${if} $R0 == 0
      ${nsProcess::KillProcess} "${APP_EXECUTABLE_FILENAME}" $R0
    ${endIf}
    ...
    IfFileExists "$LOCALAPPDATA\tnyma-ai\*.*" 0 _cu_localDone
      ...
      RMDir /r "$LOCALAPPDATA\tnyma-ai"
      IfFileExists "$LOCALAPPDATA\tnyma-ai\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\tnyma-ai"'
        Pop $0
        Pop $1
    _cu_localDone:
    ...
    IfFileExists "$APPDATA\tnyma-ai\*.*" 0 _cu_roamingDone
      ...
      RMDir /r "$APPDATA\tnyma-ai"
      IfFileExists "$APPDATA\tnyma-ai\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\tnyma-ai"'
        Pop $0
        Pop $1
    _cu_roamingDone:
