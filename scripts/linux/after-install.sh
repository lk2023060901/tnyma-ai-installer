...
# Post-installation script for TnymaAI on Linux
...
# Create symbolic link for TnymaAI app binary
if [ -x /opt/TnymaAI/tnyma-ai ]; then
    ln -sf /opt/TnymaAI/tnyma-ai /usr/local/bin/tnyma-ai 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/TnymaAI/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/TnymaAI/chrome-sandbox' || true
else
    chmod 0755 '/opt/TnymaAI/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/TnymaAI/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/tnyma-ai'
...
echo "TnymaAI has been installed successfully."
