...
<p align="center">
  <strong>AIエージェントのためのデスクトップインターフェース</strong>
</p>

<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜtnyma-aiなのか">なぜTnymaAIなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**TnymaAI**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。同梱ランタイムを基盤に、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験へ変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、TnymaAIはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

TnymaAIはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
インストーラー初回起動時には、既存の OpenClaw 系デスクトップ製品がないかも確認し、見つかった場合はアンインストールしてからセットアップを続行できるようになりました。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。TnymaAIはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**
...
TnymaAIは公式ランタイムコアをアプリケーション内に直接同梱しています。別途インストールを必要とせず、シームレスな「バッテリー同梱」体験を提供します。

私たちはアップストリームのランタイムプロジェクトとの厳密な整合性を維持することにコミットしており、公式リリースが提供する最新の機能、安定性の改善、エコシステムの互換性に常にアクセスできることを保証します。
...
`@agent` で別のエージェントを選ぶと、TnymaAI はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離はランタイムの sandbox 設定に依存します。
...
TnymaAI には Tencent 公式の個人 WeChat チャンネルプラグインも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携でき、セットアップウィザードからも同じ QR ベースの導線を開始できます。
Feishu チャンネルは、App ID / App Secret の手動入力に加えて、Channels ページから QR ログインしてボット作成、権限付与、イベント購読、公開まで自動化し、作成者へ歓迎メッセージを送るフローにも対応しました。QQ Bot チャンネルも App ID / Client Secret の手動入力を残したまま、同じ Channels ページから QR でワンクリック作成し、作成成功後に歓迎メッセージを自動送信できるようになりました。
...
TnymaAI はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）も既定で有効化されますが、必要な API キーが未設定の場合は TnymaAI が実行時に設定エラーを表示します。
...
プロバイダー設定は、ローカルで縮小したプリセット一覧ではなく、上流の auth choice カタログに合わせるようになりました。これにより、セットアップウィザードと **設定 → AI Providers** は、デスクトップ側で扱える範囲で stock runtime と同じ入口を表示します。
API キーの検証または OAuth ログインに成功すると、TnymaAI はそのプロバイダーのモデル一覧をすぐに取得し、同期済みモデル一覧をアカウントへ保存します。以後はモデル ID を手入力せずに、同期済みリストから切り替えられます。
OpenAI-compatible ゲートウェイを **Custom プロバイダー** で使う場合、**設定 → AI Providers → Provider 編集** でカスタム `User-Agent` を設定でき、互換性が必要なエンドポイントで有効です。

### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。TnymaAIはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に TnymaAI がバックグラウンドで自動起動します。
**Gateway auto-start** を有効のままにしておけば、バックグラウンド起動時に TnymaAI Gateway も自動で復元され、セットアップウィザードは再表示されません。
初回セットアップ完了後に TnymaAI を開き直した場合は、多段のセットアップウィザードではなく、同梱された TnymaAI Web ページを開く軽量な Control UI ランチャーだけが表示されます。
...
[Releases](https://github.com/ValueCell-ai/TnymaAI/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
TnymaAIを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：

1. **言語と地域** – 使用する言語と地域を設定
2. **環境チェック** – 同梱された TnymaAI ランタイムを確認し、Gateway を起動
3. **AIプロバイダー** – 上流の Provider auth choices から入口を選び、API キーまたは OAuth を検証
4. **モデル選択** – プロバイダーのモデル一覧を同期し、既定モデルを保存。取得できた場合は最初のモデルを既定で選択
5. **WeChat / Feishu / QQ ボット** – モデル設定の直後に、必要なら WeChat、Feishu、または QQ ボットを QR ワンクリックフローで設定
6. **コンポーネントのインストール** – 既定のローカルスキルをインストールしてデスクトップの初期設定を完了

サポート対象のシステム言語がある場合、ウィザードはその言語を初期選択し、未対応の場合は英語にフォールバックします。

### プロキシ設定

TnymaAIには、Electron、TnymaAI Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、TnymaAIは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- TnymaAIはTelegramが有効な場合、プロキシをランタイムのTelegramチャネル設定にも同期します。
- TnymaAIのプロキシが無効な状態では、Gatewayの通常再起動時に既存のTelegramチャネルプロキシ設定を保持します。
- ランタイム設定のTelegramプロキシを明示的に消したい場合は、プロキシ無効の状態で一度「保存」を実行してください。
...
TnymaAIは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
- TnymaAI は Electron アプリのため、**1つのアプリインスタンスでも複数プロセス（main/renderer/zygote/utility）が表示される**のが正常です。
...
- ただし TnymaAI Gateway の待受は常に**単一**であるべきです。`127.0.0.1:18789` を Listen しているプロセスは1つだけです。
- Listen プロセスの確認例:
  - macOS/Linux: `lsof -nP -iTCP:18789 -sTCP:LISTEN`
  - Windows (PowerShell): `Get-NetTCPConnection -LocalPort 18789 -State Listen`
- ウィンドウの閉じるボタン（`X`）は既定でトレイへ最小化する動作で、完全終了ではありません。完全終了する場合はトレイメニューの **Quit TnymaAI** を使用してください。
...
│   ├── gateway/             # TnymaAI ゲートウェイプロセスマネージャー
...
このリポジトリには `tnyma-ai` + `tnyma-ai-installer` 向けのローカル GitLab CI/CD 雛形が含まれます。

- GitLab Docker デプロイ: `ops/gitlab/docker-compose.yml`
- GitLab セットアップ手順: `ops/gitlab/README.md`
- インストーラー用パイプライン: `.gitlab-ci.yml`
...
タグ付きパイプラインではさらに、`TNYMA_AI_REF` を不変参照に固定し、macOS の notarization を検証し、任意で Feishu のビルド/リリース通知ボットにも送信できます。macOS runner では `package-mac.sh` 経由で `notarytool` の keychain profile を使った notarization もサポートします。
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がTnymaAIをより良くするのに役立ちます。
...
TnymaAIは優れたオープンソースプロジェクトの上に構築されています：

- [ランタイム上流プロジェクト](https://github.com/OpenClaw) – AIエージェントランタイム
...
TnymaAI パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に TnymaAI を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、TnymaAI チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/TnymaAI&type=Date" alt="スター履歴チャート" />
</p>
...
TnymaAIは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
